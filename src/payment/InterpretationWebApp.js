...
import { CITY_OPTIONS, ENGINE_INFO, generateInterpretationPreview, getBirthFormOptions, getDayOptions, getInterpretationFocusOptions, getReportOptions } from './interpretationEngineBridge';
...
function WheelColumn({ label, items, selectedValue, onSelect, width = 90 }) {
  return (
    <View style={[styles.wheelColumn, { width }]}>
      ...
      <ScrollView style={styles.wheelViewport} contentContainerStyle={styles.wheelContent} showsVerticalScrollIndicator={false}>
        {items.map((item) => {
...
function CityPickerModal({ visible, onClose, selectedCityKey, onSelect }) {
  ...
  const regions = useMemo(() => [...new Set(['China', ...CITY_OPTIONS.map((city) => city.region)])], []);
  const searchResults = useMemo(() => search.trim() ? CITY_OPTIONS.filter((city) => `${city.label} ${city.region} ${city.timezone}`.toLowerCase().includes(search.trim().toLowerCase())) : [], [search]);
  const regionCities = useMemo(() => CITY_OPTIONS.filter((city) => city.region === region), [region]);
  return (
          ...
          {search.trim() ? (
            <ScrollView style={styles.modalBody}>
              {searchResults.map((city) => {
                ...
                return <Pressable key={city.key} onPress={() => { onSelect(city.key); setRegion(city.region); setSearch(''); onClose(); }} style={[styles.searchRow, active && styles.searchRowActive]}><Text style={[styles.searchTitle, active && styles.searchTitleActive]}>{city.label}</Text><Text style={styles.searchMeta}>{city.region} · {city.timezone}</Text></Pressable>;
              })}
            </ScrollView>
          ) : (
            <View style={styles.cityColumns}>
              <ScrollView style={styles.regionList}>
                {regions.map((item) => { const active = item === region; return <Pressable key={item} onPress={() => setRegion(item)} style={[styles.regionItem, active && styles.regionItemActive]}><Text style={[styles.regionText, active && styles.regionTextActive]}>{item}</Text></Pressable>; })}
              </ScrollView>
              <ScrollView style={styles.cityList}>
                {regionCities.map((city) => { const active = city.key === selectedCityKey; return <Pressable key={city.key} onPress={() => { onSelect(city.key); onClose(); }} style={[styles.cityListItem, active && styles.cityListItemActive]}><Text style={[styles.cityListTitle, active && styles.cityListTitleActive]}>{city.label}</Text><Text style={styles.cityListMeta}>{city.timezone}</Text></Pressable>; })}
              </ScrollView>
            </View>
          )}
        </View>
...
export default function InterpretationWebApp() {
  ...
  const [generatedResult, setGeneratedResult] = useState(() => generateInterpretationPreview(buildInput({ reportType: 'past-life', year: '1994', month: '09', day: '17', hour: '08', minute: '30', cityKey: 'perth', focus: 'self' })));
            ...
            <>
              ...
              <View style={[styles.workspace, isNarrow && styles.workspaceStack]}>
                <View style={styles.formPanel}>
                  ...
                  <Pressable style={styles.cityTrigger} onPress={() => setCityPickerVisible(true)}><View><Text style={styles.cityTriggerTitle}>{selectedCity.label}</Text><Text style={styles.cityTriggerMeta}>{selectedCity.region} · {selectedCity.timezone}</Text></View><Text style={styles.cityTriggerAction}>选择</Text></Pressable>
                ...
                <View style={styles.resultPanel}>
                  ...
                  {resultTab === 'themes' ? <View style={styles.resultCard}><Text style={styles.boxTitle}>结构化主题</Text><View style={styles.tagWrap}>{generatedResult.tags.map((tag) => <View key={tag.code} style={styles.tag}><Text style={styles.tagText}>{tag.label}</Text></View>)}</View><View style={styles.innerCard}><Text style={styles.boxTitle}>关键相位</Text>{generatedResult.chartVisual.aspects.map((aspect) => <View key={aspect.code} style={styles.detailRow}><Text style={styles.detailTitle}>{aspect.label}</Text><Text style={styles.detailBody}>Orb {aspect.orb}</Text></View>)}</View></View> : null}
                  {resultTab === 'details' ? <View style={styles.resultCard}><Text style={styles.boxTitle}>详细星盘解释</Text>{generatedResult.detailSections.map((section) => <View key={section.title} style={styles.detailRow}><Text style={styles.detailTitle}>{section.title}</Text><Text style={styles.detailBody}>{section.body}</Text></View>)}</View> : null}
                </View>
              </View>
            </>
          ) : null}
...
const styles = StyleSheet.create({
  ...
  section: { paddingHorizontal: 20, paddingTop: 24 }, sectionEyebrow: { color: '#516b62', fontSize: 12, lineHeight: 16, fontWeight: '800', textTransform: 'uppercase' }, sectionTitle: { marginTop: 6, color: '#17221d', fontSize: 28, lineHeight: 34, fontWeight: '800', maxWidth: 760 }, grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 14, marginTop: 14 }, card: { minWidth: 240, flexGrow: 1, flexBasis: 240, borderRadius: 8, backgroundColor: '#fbfdfc', borderWidth: 1, borderColor: 'rgba(23,34,29,0.1)', padding: 18 }, cardTitle: { color: '#17221d', fontSize: 20, lineHeight: 24, fontWeight: '800' }, cardBody: { marginTop: 10, color: '#5a6b64', fontSize: 15, lineHeight: 22 },
  ...
  wheelRow: { marginTop: 10, flexDirection: 'row', flexWrap: 'wrap', gap: 12 }, wheelColumn: { gap: 8 }, wheelLabel: { color: '#5a6b64', fontSize: 12, lineHeight: 16, fontWeight: '700' }, wheelViewport: { height: 192, borderRadius: 8, borderWidth: 1, borderColor: 'rgba(23,34,29,0.12)', backgroundColor: '#fff' }, wheelContent: { paddingVertical: 8 }, wheelItem: { minHeight: 42, alignItems: 'center', justifyContent: 'center', marginHorizontal: 8, marginVertical: 4, borderRadius: 8 }, wheelItemActive: { backgroundColor: '#17362f' }, wheelText: { color: '#264038', fontSize: 15, lineHeight: 18, fontWeight: '700' }, wheelTextActive: { color: '#f7faf8' },
