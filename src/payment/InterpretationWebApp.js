...
function CityPickerModal({ visible, onClose, selectedCityKey, onSelect }) {
  ...
  const searchResults = useMemo(() => search.trim() ? CITY_OPTIONS.filter((city) => `${city.label} ${city.province || ''} ${city.region} ${city.timezone}`.toLowerCase().includes(search.trim().toLowerCase())) : [], [search]);
