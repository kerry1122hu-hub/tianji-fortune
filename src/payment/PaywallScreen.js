...
function pickScreenshotFile() {
  if (Platform.OS !== 'web' || typeof document === 'undefined') {
    return Promise.reject(new Error('当前环境暂不支持文件上传，请先用 Web/PWA 版测试。'));
  }
...
export function PaywallScreen({ visible, onClose, onSaveRegistration, profile, registrationDraft }) {
  ...
  const handleSubmit = async () => {
    ...
    Alert.alert('已提交审核', '付款截图和联系方式已经提交，后台确认后会为你开通权益。');
    onClose?.();
  };
      ...
      <View style={s.root}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 196 }}>
          <View style={[s.hero, { paddingTop: insets.top + 16 }]}>
            ...
            <Text style={s.heroEyebrow}>测试版收款</Text>
            <Text style={s.heroTitle}>先扫码付款，再上传截图，后台人工开通会员。</Text>
            <Text style={s.heroSubtitle}>
              这是最省事的内测收款版本。先验证有没有人真的愿意付钱，再把自动支付、自动开通和退款链路做完整。
            </Text>
          </View>

          <View style={s.section}>
            <Text style={s.sectionTitle}>先选方案</Text>
            {PLAN_OPTIONS.map((plan) => (
              ...
              >
                <View style={s.planMain}>
                  ...
                  <Text style={s.planSubtitle}>先按这一档收款，后台确认后手工开通。</Text>
                </View>
          ...
          <View style={s.section}>
            <Text style={s.sectionTitle}>再选付款方式</Text>
        ...
        <View style={[s.bottomBar, { paddingBottom: insets.bottom + 10 }]}>
          <View style={s.bottomCopy}>
            <Text style={s.bottomTitle}>提交后会进入待审核名单。</Text>
            <Text style={s.bottomBody}>后台确认截图和到账后，可以一键为你开通会员。</Text>
          </View>
